...
vec4	ComputeDirectionalLight(int iLight)
{
	...
	vec4 specular = lightsData[iLight].lightSpecular * specularStrength * texture(specularMap, vs_texCoords);

	// Emission
	vec4 emission = texture(emissionMap, vs_texCoords) * (1 - diffuseStrength);
...
vec4	ComputePointLight(int iLight, vec4 lightDirEye, float attenuation)
{
	...
	return ((ambient + diffuse + specular) * attenuation);// + emission;
}
...
vec4	ComputeSpotLight(int iLight, vec4 lightDirEye, float attenuation)
{
	...
	return ((ambient + diffuse + specular) * attenuation * intensity);// + emission;
}
