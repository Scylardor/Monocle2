...
void main()
{
	...
	for(uint i = 0u; i < SAMPLE_COUNT; ++i)
	{
		...
		vec3 H = ImportanceSampleGGX(Xi, N, PBR.roughness);
		...
		if(NdotL > 0.0)
		{
			...
			// TODO: should be parameterized... (or use textureSize and not be hardcoded)
			float resolution = 512.0;
			float saTexel  = 4.0 * PI / (6.0 * resolution * resolution);
			float saSample = 1.0 / (float(SAMPLE_COUNT) * pdf + 0.0001);

			float mipLevel = PBR.roughness == 0.0 ? 0.0 : 0.5 * log2(saSample / saTexel);
